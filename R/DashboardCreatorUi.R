...
configCreatorUi <- function(request) {
  ui <- shiny::fluidPage(
    ...
    shiny::fluidRow(
      shiny::column(
        width = 6,
        shiny::textInput("name", label = "Dashboard Name", width = "100%")
      ),
      shiny::column(
        width = 6,
        shiny::textInput("shortName", label = "Short Name", width = "100%")
      )
    ),
    shiny::fluidRow(
      shiny::column(
        width = 12,
        shiny::textAreaInput("description", label = "Description", resize = "none", width = '100%')
      )
    ),
    shiny::fluidRow(
      shiny::column(
    ...
    shiny::fluidRow(
      shiny::column(
        ...
        shinycssloaders::withSpinner(reactable::reactableOutput("dataSourceSelector"))
      )
    ),
    shiny::fluidRow(
      shiny::column(
        ...
        shinycssloaders::withSpinner(reactable::reactableOutput("analysisTypeSelector"))
      )
    ),
    shiny::fluidRow(
      shiny::column(
        ...
        shinycssloaders::withSpinner(reactable::reactableOutput("cohortSelectorTable"))
      )
    ),
    shiny::fluidRow(
      shiny::column(
        width = 3,
        shiny::actionButton("previewConfig", "Preview configuration")
      )
    ),
    shiny::fluidRow(
      shiny::column(
...
configCreatorServer <- function(input, output, session) {
    ...
    shiny::showModal(
      shiny::modalDialog(
        ...
        footer = shiny::fluidRow(
          shiny::column(width = 2, shiny::downloadButton("saveConfig", "Save")),
          shiny::column(width = 1, shiny::actionButton("closeModal", "Close")),
        )
      )
    )
  })
...
#' @export
createDashboardConfiguration <- function(configPath,
  ...
  shiny::shinyApp(server = configCreatorServer, ui = configCreatorUi, onStart = function() {
